...
def query(mode,savetable = False):
    """
    ...
    if savetable and  mode == 'training':
        
        filename = "GaiaData/TRAININGData_15kstars_15kgalaxies_15kquasars.fits"
        table = Table.from_pandas(combined) 
        table.write(filename, format="fits", overwrite=True)
        
    if savetable and  mode == 'testing':
        
        filename = "GaiaData/TESTINGData_150kstars_150kgalaxies_150kquasars.fits"  
        table = Table.from_pandas(combined) 
        table.write(filename, format="fits", overwrite=True)
